...
**Novelty score** — `turnover × recency`, both in `0..1`. For anon users turnover is flat `1.0` — their score is _pure recency_, so the tier depends on reset age alone (volume only breaks same-day sort ties). For returning users `turnover = unseenResets / (unseenResets + 0.5)` where an _unseen reset_ is any reset row logged after your last visit; each row — a named sector's drop, or "part of the gym" for unnamed gyms — is one chunk, boulder counts are deliberately ignored, and the curve never caps (every extra unseen reset adds, with diminishing returns). `recency = 0.5 ^ (daysSinceNewestUnseenReset / halfLife)` with a 7-day half-life for anon users and 14-day for returning users (the unseen count dominates; recency is only a discount). Anon users count as having last visited 28 days ago for selecting unseen resets. The home-page sort key. See [ADR-0003](docs/adr/0003-turnover-times-recency-scoring.md) for the full formula and constant rationale (supersedes ADR-0002).

**Tier** — one of `hot` / `fresh` / `worth` / `slim` / `stale` / `unknown`, bound from fixed cuts of the 0..1 Novelty score (`hot` ≥ 0.85, `fresh` ≥ 0.70, `worth` ≥ 0.50, `slim` > 0, `stale` = 0, `unknown` = no reset data). Anon score is pure recency, so the anon tier tracks reset age directly: 0–1 days → `hot`, 2–3 → `fresh`, 4–7 → `worth`, 8+ → `slim`. Each tier has user-facing copy ("sending hot", "looking fresh", "worth a climb", etc.), an emoji, and a CSS token set. UI never speaks raw scores — it speaks Tiers.
