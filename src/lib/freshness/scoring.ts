...
// Scoring model (see ADR-0004). One primitive read through two lenses.
//
...
// a reset today adds 1.0, one a half-life old adds 0.5, older ones fade toward 0.
  ...
  for (const section of sections) {
    ...
    for (const reset of section.resets) {
      if (Date.parse(reset.reset_on) > cutoffTime) {
        freshResetCount += 1;
        noveltyScore += recencyWeight(reset.reset_on);
...
// A single reset's contribution to the sum: 1.0 today, halving every
// HALF_LIFE_DAYS. Clamped at age 0 so a reset dated today never exceeds 1.0.
function recencyWeight(resetISO: string): number {
